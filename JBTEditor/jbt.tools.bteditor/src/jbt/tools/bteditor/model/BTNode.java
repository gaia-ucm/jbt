...
public class BTNode extends Observable implements Serializable {
	...
	public Identifier(String id) {
		...
		throw new RuntimeException(
			"Invalid String representation for an Identifier: "
				+ id, e);
	    }
	}
	...
	if (this.conceptualNode.getHasName()
		&& this.conceptualNode.getType().equals(
			NodeInternalType.ROOT.toString())) {
	...
	if (this.conceptualNode.getNumChildren() != 0) {
		...
		if (this.children.size() != this.conceptualNode
			.getNumChildren()) {
		    this.errorMessage = "Must have exactly "
			    + this.conceptualNode.getNumChildren()
			    + " children";
		    result = false;
		}
	    }
	...
	if (conceptualNode.getParameters().size() != 0) {
		...
		for (int i = 0; i < conceptualNode.getParameters().size(); i++) {
			...
			if (this.parameters.get(i).getFromContext()) {
				...
				this.errorMessage = "Invalid value for parameter "
					+ p.getName();
				result = false;
			    }
			} else {
			    if (ParameterType.isVariable(p.getType())) {
				BTNode.VarParameter varParameter = (BTNode.VarParameter) this.parameters
					.get(i);
				String defaultTextValue = varParameter
					.getVariableName();
				if (!checkVarParameter(p, actualValue,
					defaultTextValue,
					varParameter.getIsConstant(), this.tree)) {
				    this.errorMessage = "Invalid value for parameter "
					    + p.getName();
				    result = false;
				}
	...
	if (this.guard != null) {
	    if (!this.guard.check()) {
		this.errorMessage = "Error in the guard: "
			+ this.guard.errorMessage;
		result = false;
	    }
	}
	...
	if (type == ParameterType.INTEGER) {
		...
		try {
		    ...
		    /*
		     * Check that node exists and that its type is compatible
		     * with the types that the parameter supports.
		     */
     ...
     * parameter. This is done this way to ensure that both the fields of the
     * key-value pair are well formed the {@link ParameterType#NODE_ID}).
     */
    public static boolean checkVarParameter(
	    jbt.tools.bteditor.model.ConceptualBTNode.Parameter parameterDefinition,
	    String value, String variableName, boolean isConstant, BT tree) {
	...
	return "ID: " + this.ID + " - Type: "
		+ this.conceptualNode.getReadableType() + " - Num. Children: "
		+ this.children.size();
    }
}

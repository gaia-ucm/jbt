...
public class BT {
    ...
    private BTNode root;
    /**
     * Counter of how many nodes have been created through the
     * {@link #createNode(ConceptualBTNode)} . This counter is used when
     * generating ids for nodes of this tree.
     */
    ...
    /**
     * Finds a node in the tree, by using an identifier. Returns null if not
     * found.
     */
    ...
    /**
     * Creates a BTNode from a ConceptualBTNode for this tree. This method
     * creates a BTNode and sets its ID to an unique identifier for the tree.
     * Also it sets the underlying conceptual node of the BTNode to
     * <code>conceptualNode</code>.
     */
    ...
    /**
     * This method must be called when there is a change in the tree. By doing
     * so, all entities that are listening to changes in the tree will be
     * notified.
     */
    ...
    /**
     * Checks the validity of all the nodes of the tree. Returns a list with the
     * nodes that contain errors.
     */
    ...
    /**
     * Given a BTNode, this method recompute its ID as well as the IDs of all
     * its children and guards. After calling this method, these IDs will not
     * clash any of those of this BT's nodes. This method can be used, for
     * example, when a whole branch must be added to this tree. In such case,
     * this method can be used to recompute the IDs of the nodes of the branch
     * so that there is no conflict among IDs.
     * <p>
     * It is important to notice that after recomputing the IDs, they are still
     * consistent among each other. For instance, if before recomputing the ID a
     * node <i>A</i> had ID "Node_34", and it referenced by another node
     * <i>B</i>, after recomputing IDs the reference will still be valid (it
     * will not be "Node_34", but whatever new ID <i>A</i> has will be properly
     * referenced by <i>B</i>).
     * 
     * @param node
     *            the root of the branch (tree) whose nodes will get their IDs
     *            recomputed.
     */
    ...
    /**
     * Given a BTNode, this method sets, as the current underlying BT of
     * <code>node</code> and all of its descendants (including guards), this BT.
     * 
     * @param node
     *            the root of the branch (tree) whose underlying BT will be
     *            modified.
     */
    ...
    /**
     * Reassigns the all the IDs in the branch whose root is <code>node</code>.
     * This method just adds <code>offset</code> to the value of all the nodes
     * (including guards) of the branch.
     */
	...
	if (node.getParameters().size() != 0) {
		...
		Parameter cParameter = node.getConceptualNode().getParameters()
			.get(i);
		if (cParameter.getType() == ParameterType.NODE_ID) {
		    Identifier newValue = new Identifier(node.getParameters()
			    .get(i).getValue());
    ...
    /**
     * Computes the largest ID of a whole branch of a BT (including guards). The
     * root of the branch is <code>node</code>.
     */
	...
	if (node.getGuard() != null) {
	    Identifier guardLargestIdentifier = computeLargestID(node
		    .getGuard());
    ...
    /**
     * Stores into <code>largestKnownID</code> (which initially must be a valid
     * Identifier for the BT) the largest Identifier of the nodes in
     * <code>node</code> (including guards).
     */
    ...
    /**
     * Computes the lowest ID of a whole branch of a BT (including guards). The
     * root of the branch is <code>node</code>.
     */
    ...
    /**
     * Stores into <code>lowestKnownID</code> (which initially must be a valid
     * Identifier for the BT) the lowest Identifier of the nodes in
     * <code>node</code> (including guards).
     */
    ...
    /**
     * Checks the validity of all the nodes of the tree whose root is
     * <code>node</code>, and stores those that are incorrect into
     * <code>ids</code>.
     */
    ...
    /**
     * Finds a node by identifier, starting the search in
     * <code>currentNode</code>. This is a recursive method that is called again
     * on the children. Returns null if the node cannot be found.
     */
     ...
     * {@link #createNode(ConceptualBTNode)}, so that the counter is bigger than
     * the biggest number associated to any of the node's identifiers in the
     * tree. This method should be called when the tree gets removed or added
     * nodes. Note that it is automatically called when {@link #setRoot(BTNode)}
     * is called.
     */
    ...
    /**
     * Updates the field {@link #nodeCounter} so that the counter is bigger than
     * the biggest number associated to any of the identifiers of descendants of

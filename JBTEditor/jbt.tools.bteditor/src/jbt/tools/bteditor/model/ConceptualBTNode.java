...
public class ConceptualBTNode implements Serializable {
	...
	public static boolean isVariable(ParameterType type) {
	    if (type == VARIABLE_INT || type == VARIABLE_FLOAT
	 ...
	 * allowed types. If an empty list is returned, it means all kinds are
	 * allowed.
	 */
	 ...
	 * When a parameter is a reference to a node (
	 * {@link ParameterType#NODE_ID}), that node may have to be of a
	 * particular kind, in which case this method adds one allowed type.
	 */
	...
	copy.readableType = this.readableType == null ? null : new String(
		this.readableType);

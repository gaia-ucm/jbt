...
public class NodeInfoViewer extends Composite implements Observer {
	...
	if (!this.node.getConceptualNode().getType()
		.equals(NodeInternalType.ACTION.toString())
	...
	this.toolkit.createLabel(parent, this.node.getID().toString(),
		SWT.RIGHT).setLayoutData(
	...
	if (this.node.getConceptualNode().getHasName()) {
		...
		valueLabel.setText(this.node.getConceptualNode()
			.getReadableType());
	    }
	}
	...
	for (Parameter p : parameters) {
	    ...
	    valueLabel = this.toolkit.createLabel(parent, p.getValue(),
		    SWT.RIGHT);
	    valueLabel
		    .setLayoutData(new TableWrapData(TableWrapData.FILL_GRAB));

	    nameLabel.setText(p.getName()
		    + (p.getFromContext() ? " (from context)" : ""));
	}

	/* Show error message. */
	if (this.node.getErrorMessage() != null) {
	    this.toolkit.createLabel(parent, "ERROR");
	    this.toolkit.createLabel(parent, this.node.getErrorMessage(),
		    SWT.RIGHT).setLayoutData(
		    new TableWrapData(TableWrapData.FILL_GRAB));
	}

...
public class BTXMLManager {
     ...
     * Exports a {@link BT} into an XML file.
     */
    public static void export(BT tree, FileOutputStream file)
	    throws IOException {
	 ...
	 */
	ConceptualBTNode xmlModel = NodesLoader.getNode(
		e.getAttributeValue("type"), e.getAttributeValue("name"));
	if (xmlModel == null) {
	    throw new IOException("Could not find type "
	...
	if (result.getConceptualNode().getType()
	...
	if (parameters != null) {
		...
		if (fromContextString.equals("true")) {
		    ...
		    throw new IOException("Unexpected \"fromcontext\" value: "
			    + fromContextString);
		}

		actualParameter.setFromContext(fromContext);

		ConceptualBTNode.Parameter par = xmlModel.getParameters()
			.get(i);

		if (ParameterType.isVariable(par.getType())) {
		    VarParameter varParameterResult = new VarParameter(
			    actualParameter);
		    boolean isConstant = p.getAttributeValue("isConstant")
			    .equals("true") ? true : false;
		    varParameterResult.setIsConstant(isConstant);

		    if (!isConstant)
			varParameterResult.setVariableName(p
	...
	for (Parameter p : node.getParameters()) {
	    ...
	    currentParam.setAttribute("fromcontext",
		    p.getFromContext() ? "true" : "false");

	    ConceptualBTNode.Parameter aux = node.getConceptualNode()
		    .getParameters().get(i);
		...
		currentParam.setAttribute("isConstant", isConstant ? "true"
			: "false");

		if (!isConstant) {
		    currentParam.setAttribute("variableName",
			    varParameter.getVariableName());
		}
	    }
	    currentParam.setText(p.getValue());

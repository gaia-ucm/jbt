 ...
 * 
 *         The main purpose of the modification is to add the functionality of
 *         register all the standard nodes paired with their category in a
 *         hashtable When the c++ file is written, that info would be needed in
 *         order to separate the different node types by their category
 */
public class NodesLoader {
     ...
     */
    public static ConceptualNodesTree loadNonStandardNodes(String fileName)
	    throws IOException {
	...
	try {
	    ...
	    List<ParsedAction> actions = parsedDomain.getActionSet()
		    .getAction();

	    List<ParsedMethod> conditions = parsedDomain.getSensorSet()
		    .getMethods();

	    for (ParsedAction action : actions) {
		ConceptualBTNode xmlAction = loadNonStandardAction(action);
		nonStandardNodes.put(
			new Pair<String, String>(xmlAction.getType(), action
				.getName()), xmlAction);
		tree.insertNode(file.getName()
			+ ConceptualNodesTree.CATEGORY_SEPARATOR + "Actions",
		...
		if (condition.getReturnedType() == ActionParameterType.BOOLEAN) {
		    ...
		    nonStandardNodes.put(
			    new Pair<String, String>(xmlCondition.getType(),
				    condition.getName()), xmlCondition);
		    tree.insertNode(file.getName()
			    + ConceptualNodesTree.CATEGORY_SEPARATOR
			    + "Conditions", new ConceptualBTNodeItem(
			    xmlCondition));
		}
	    }
	...
	return nonStandardTrees.keySet().toArray(
		new String[nonStandardTrees.size()]);
    }
    ...
    private static ConceptualBTNode loadNonStandardCondition(
	    ParsedMethod condition) {
	...
	for (ParsedActionParameter parameter : parameters) {
	    ...
	    xmlNodeParameter
		    .setType(fromMMPMParameterType(parameter.getType()));
	...
	if (nodeType.equals("Category")) {
		...
		String nextPath = currentPath == null ? categoryName
			: currentPath + ConceptualNodesTree.CATEGORY_SEPARATOR
				+ categoryName;
	    ...
	    xmlNode.setNumChildren(numChildren.equals("I") ? -1 : Integer
	    ...
	    ConceptualBTNodeItem nodeToInsert = new ConceptualBTNodeItem(
		    xmlNode);
	...
	if (nodesByCategory.containsKey(type)) {
	    ...
	    String parentType = category
		    .split(ConceptualNodesTree.CATEGORY_SEPARATOR)[1];
	...
	if (type == ParameterType.NODE_ID) {
		...
		throw new RuntimeException(
			"Invalid value for \"contextable\" attribute");
	    }
	}

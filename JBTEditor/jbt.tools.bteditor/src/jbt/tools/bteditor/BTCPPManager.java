...
public class BTCPPManager {
     ...
     */
    public static void exportInline(BT tree, FileOutputStream file)
	    throws IOException {
     ...
     */
    public static void exportHeader(BT tree, FileOutputStream file)
	    throws IOException {
	...
	if (!(node.getConceptualNode().getType().equals(NodeInternalType.ROOT
		.toString()))) {
	    p.print(endToken);
	} else {
	    p.print(margin + ";");
    ...
    /**
     * Creates the method calls associated to all the parameters of a
     * {@link BTNode}.
     */
    private static void processParameters(BTNode node, PrintStream p,
	    String margin) {

	for (int i = 0; i < node.getParameters().size(); ++i) {
	    Parameter par = node.getParameters().get(i);
	    ParameterType type = node.getConceptualNode().getParameters()
		    .get(i).getType();
	    String textToWrite = "";
		...
		if (!varParameter.getIsConstant())
		    textToWrite = "IntParam" + "(\""
			    + varParameter.getVariableName() + "\", "
			    + varParameter.getValue() + ")";
		else
		...
		if (!varParameter.getIsConstant())
		    textToWrite = "FloatParam" + "(\""
			    + varParameter.getVariableName() + "\", "
			    + Float.parseFloat(varParameter.getValue()) + "f)";
		else
		    textToWrite = ""
			    + Float.parseFloat(varParameter.getValue()) + "f";

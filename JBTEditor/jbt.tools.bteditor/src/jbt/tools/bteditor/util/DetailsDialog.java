...
public class DetailsDialog extends IconAndMessageDialog {
    ...
    public DetailsDialog(String title, String message, String details,
	    int iconType, Shell parentShell) {
	this(title, message, details, iconType, parentShell, SWT.CLOSE
     ...
     */
    public DetailsDialog(String title, String message, String details,
	    int iconType, Shell parentShell, int shellStyle) {
	super(parentShell);
	if (message == null) {
	...
	if (buttonId == IDialogConstants.DETAILS_ID) {
		...
		this.detailsText.setBackground(Display.getCurrent()
	     ...
	     */
	    Point dialogNewDimensions = getShell().computeSize(SWT.DEFAULT,
		    SWT.DEFAULT);
	    int screenHeight = Display.getCurrent().getClientArea().height;
	    getShell().setSize(
		    new Point(dialogOldDimensions.x, Math.min(
			    dialogNewDimensions.y, screenHeight)));
	} else {
	    /*
	     * Close the dialog...
	     */
	...
	default:
	    throw new RuntimeException("Invalid type of image: "
		    + this.iconType);
	}
    }

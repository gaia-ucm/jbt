...
public class BTEditor extends EditorPart implements ITreeModifierListener {
     ...
     */
    private Color ERROR_COLOR = Utilities.getDisplay().getSystemColor(
	    SWT.COLOR_RED);
    /**
	...
	try {
		...
		new SaveBTAction(this.tree,
			((BTEditorInput) getEditorInput()).getTreeName()).run();
		this.dirty = false;
		...
		if (guard != null) {
		    ...
		    BTEditorInput editorInput = (BTEditorInput) this
			    .getEditorInput();
		    Utilities.getBTEditor(Long.parseLong(editorInput
			    .getTreeName().split(File.pathSeparator)[0])).viewer
			    .refresh();
		}
		this.dirty = false;
		firePropertyChange(EditorPart.PROP_DIRTY);
	...
	SaveBTAsAction action = new SaveBTAsAction(this.tree, this
	...
	IPartService partService = (IPartService) this.getSite().getService(
		IPartService.class);
	...
	if (editorInput.isFromFile()) {
		...
		throw new PartInitException(
			"There were errors while loading the tree: "
				+ e.getMessage(), e);
	    }
	    ...
	    String[] pieces = editorInput.getTreeName().split(
		    File.pathSeparator);
	    ...
	    this.tree = new BT();
	    this.tree.addTreeModifiedListener(this);
	    tree.setRoot(tree.createNode(NodesLoader.getNode(
	...
	this.viewer
		.addSelectionChangedListener(new ISelectionChangedListener() {
			...
			NodeInfo nodeInfoView = (NodeInfo) Utilities
				.getView(NodeInfo.class);
	...
	/* Adding drop support. */
	this.viewer.addDropSupport(DND.DROP_MOVE, new Transfer[] {
		ConceptualBTNodeTransfer.getInstance(),
	...
	treeWidget.addMenuDetectListener(new MenuDetectListener() {
		...
		if (selectedNodes.size() != 0) {
		    ...
		    treeWidget.setMenu(menuManager
			    .createContextMenu(treeWidget));
		}
	    }
	});
	...
	this.viewer.addDoubleClickListener(new IDoubleClickListener() {
		...
		if (!selection.isEmpty()) {
			...
			new NodeParametersDialog(Utilities.getShell(),
				selectedNode).open();
		    }
		}
	    }
	...
	public void run() {
		...
		List<BTNode> selectedElements = activeEditor
			.getSelectedElements();
		if (selectedElements.size() == 1) {
			...
			if (this.selectedNode.getChildren().size() != 0) {
			    this.selectedNode = this.selectedNode.getChildren()
				    .get(0);
			} else {
		...
		if (this.selectedNode != null) {
		    BTEditorCopyAndPasteManager.getInstance().copy(
			    this.selectedNode);
		}
	    }
	}
	...
	public void run() {
		...
		if (selectedElements.size() == 1) {
		    ...
		    if (this.selectedNode.getConceptualNode().getNumChildren() != -1
			    && this.selectedNode.getConceptualNode()
				    .getNumChildren() <= this.selectedNode
				    .getNumChildren()) {
			this.selectedNode = null;
		    }
		}
		...
		if (this.selectedNode != null) {
		    BTNode pastedNode = BTEditorCopyAndPasteManager
			    .getInstance().paste();
	...
	public void run() {
	    ...
	    if (this.selectedNode != null) {
		BTNode pastedNode = BTEditorCopyAndPasteManager.getInstance()
			.paste();
	...
	protected Control createDialogArea(Composite parent) {
		...
		public Point computeSize(int wHint, int hHint, boolean changed) {
		    return new Point(wHint < 200 ? 200 : wHint, guardsViewer
			    .getTree().getItemHeight() * 2);
		}
	    };

	    guardsViewerComposite.setLayoutData(new GridData(SWT.FILL,
		    SWT.FILL, true, true));
	    ...
	    this.guardsViewer
			...
			public void doubleClick(DoubleClickEvent event) {
				...
				if (node.getGuard().getChildren().size() != 0) {
					...
					StandardDialogs
						.exceptionDialog(
							"Error opening the guard",
							"There was an unexpected error when opening the guard",
							ex);
				    }
				} else if (node.getGuard().getConceptualNode()
					.getParameters().size() != 0) {
				    new NodeParametersDialog(getShell(), node
					    .getGuard()).open();
				}
			    }
			}
	    ...
	    buttonsComposite.setLayoutData(new GridData(SWT.FILL, SWT.TOP,
		    false, false));
	...
	private Button createAddSimpleGuardButton(Composite parent) {
	    ...
	    addSimpleGuardButton.setLayoutData(new GridData(SWT.FILL, SWT.TOP,
		    true, false));
		...
		public void widgetSelected(SelectionEvent e) {
		    new GuardInsertionDialog(Utilities.getShell(), node,
			    guardEditionDialog).open();
		}
	    });
	...
	private Button createAddComplexGuardButton(Composite parent) {
	    ...
	    addGuardButton.setLayoutData(new GridData(SWT.FILL, SWT.TOP, true,
		    false));
	...
	private Button createEditGuardButton(Composite parent) {
	    ...
	    editGuardButton.setLayoutData(new GridData(SWT.FILL, SWT.TOP, true,
		    false));
		...
		public void widgetSelected(SelectionEvent e) {
			...
			if (node.getGuard().getChildren().size() != 0) {
				...
				StandardDialogs
					.exceptionDialog(
						"Error when creating new tree",
						"There was an unexpected error when creating the tree",
						ex);
			    }
			} else if (node.getGuard().getConceptualNode()
				.getParameters().size() != 0) {
	...
	private Button createRemoveGuardButton(Composite parent) {
	    ...
	    removeGuardButton.setLayoutData(new GridData(SWT.FILL, SWT.TOP,
		    true, false));
	...
	protected Control createDialogArea(Composite parent) {
	     ...
	     */
	    Composite leafNodesViewerComposite = new Composite(composite,
		    SWT.NONE) {
		public Point computeSize(int wHint, int hHint, boolean changed) {
		    return new Point(leafNodesViewer.getTable().computeSize(
			    SWT.DEFAULT, SWT.DEFAULT).x, leafNodesViewer
			    .getTable().getItemHeight() * 7);
		}
	    };

	    leafNodesViewerComposite.setLayoutData(new GridData(SWT.FILL,
		    SWT.FILL, true, true));
	    leafNodesViewerComposite.setLayout(new FillLayout());
	    this.leafNodesViewer = new TableViewer(leafNodesViewerComposite,
		    SWT.BORDER | SWT.SINGLE);
	    this.leafNodesViewer
		    .setContentProvider(new ConceptualBTNodeListContentProvider());
	    this.leafNodesViewer
		    .setLabelProvider(new ConceptualBTNodeListLabelProvider());

	    /*
	     * Build a list with all the nodes that can be used as guards, which
	     * are leaf nodes.
	     */
	    List<ConceptualBTNode> standardNodes = NodesLoader
		    .getStandardNodes();
	    List<ConceptualBTNode> nonStandardNodes = NodesLoader
		    .getNonStandardNodes();
		...
		public int compare(Viewer viewer, Object e1, Object e2) {
		    ...
		    if (node1Type.equals(NodeInternalType.ACTION.toString())
			    && node2Type.equals(NodeInternalType.CONDITION
				    .toString())) {
			return 1;
		    }

		    if (node1Type.equals(NodeInternalType.CONDITION.toString())
			    && node2Type.equals(NodeInternalType.ACTION
				    .toString())) {
			return -1;
		    }

		    if (!node1Type.equals(NodeInternalType.ACTION.toString())
			    && !node1Type.equals(NodeInternalType.CONDITION
				    .toString())) {
			if (!node2Type.equals(NodeInternalType.ACTION
				.toString())
				&& !node2Type.equals(NodeInternalType.CONDITION
					.toString())) {
			    return node1.getReadableType().compareToIgnoreCase(
	 ...
	 * 
	 */
	private class ConceptualBTNodeListContentProvider implements
	 ...
	 * 
	 */
	private class ConceptualBTNodeListLabelProvider implements
		ILabelProvider {
	...
	protected Control createDialogArea(Composite parent) {
	    ...
	    if (this.node.getConceptualNode().getType()
		    .equals(NodeInternalType.ROOT.toString())) {
		this.nameEditor = new NameEditorComposite(composite, SWT.NONE,
			this.node.getName());
		this.nameEditor.setLayoutData(new GridData(SWT.FILL, SWT.TOP,
			true, false));
	    }
		...
		parametersGroup.setLayoutData(new GridData(SWT.FILL, SWT.FILL,
			true, true));
		parametersGroup.setLayout(new GridLayout(1, false));
		this.nodeEditor = new ParametersEditorComposite(
			parametersGroup, SWT.NONE, this.node);
		this.nodeEditor.setLayoutData(new GridData(SWT.FILL, SWT.FILL,
			true, true));
	    }
	...
	protected void buttonPressed(int buttonId) {
		...
		if (this.nodeEditor != null) {
		    try {
			List<Parameter> params = this.nodeEditor
				.getParameters();
			...
			MessageDialog.openError(this.getShell(),
				"Error validating parameters", e.getMessage());
			return;
		    }
		}
		if (this.nameEditor != null) {
			...
			MessageDialog.openError(this.getShell(),
				"Error validating name", e.getMessage());
			return;
		    }
		}
	 ...
	 * displayed.
	 */
	public NameEditorComposite(Composite parent, int style,
		String initialName) {
	    super(parent, style);
	    ...
	    this.nameLabel.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER,
		    false, false));

	    this.nameValue = new Text(this, SWT.BORDER);
	    this.nameValue.setLayoutData(new GridData(SWT.FILL, SWT.CENTER,
		    true, false));
	...
	public ParametersEditorComposite(Composite parent, int style,
		BTNode node) {
	    super(parent, style);
	    this.node = node;
		...
		String currentParameterValue = this.node.getParameters().size() > 0 ? this.node
			.getParameters().get(i).getValue()
			: null;

		boolean currentFromContext = this.node.getParameters().size() > 0 ? this.node
			.getParameters().get(i).getFromContext()
			: false;

		boolean isConstant = true;
		if (ParameterType.isVariable(this.node.getConceptualNode()
			.getParameters().get(i).getType())) {
		    String fieldValue = null;
		    BTNode.VarParameter varNodeParameter = null;
		    if (this.node.getParameters().size() > 0) {
			varNodeParameter = (BTNode.VarParameter) this.node
				.getParameters().get(i);
			fieldValue = varNodeParameter.getVariableName();
			isConstant = varNodeParameter.getIsConstant();
		    }
		    VarParameterComposite pc = new VarParameterComposite(this,
			    SWT.NONE, parameters.get(i), currentParameterValue,
			    currentFromContext, isConstant, fieldValue,
			    varNodeParameter);
		    this.parameterComposites.add(pc);
		    pc.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
			    false));
		} else {
		    ParameterComposite pc = new ParameterComposite(this,
			    SWT.NONE, parameters.get(i), currentParameterValue,
			    currentFromContext);
		    this.parameterComposites.add(pc);
		    pc.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
	 ...
	 */
	public ParameterComposite(Composite parent, int style,
		jbt.tools.bteditor.model.ConceptualBTNode.Parameter parameter,
		String initialValue, boolean initialFromContext) {
	    super(parent, style);
		...
		this.fromContextButton.setLayoutData(new GridData(SWT.RIGHT,
	...
	public Parameter getParameter() throws RuntimeException {
	     ...
	     */
	    if (this.parameter.getContextable()
		    && this.fromContextButton.getSelection()) {
		...
		if (!BTNode.checkParameter(parameter, this.valueText.getText(),
			tree)) {
	...
	private String getTooltip(
		...
		if (parameterDefinition.getNodeClasses().size() != 0) {
			...
			validTypes += NodesLoader.getNode(type, null)
				.getReadableType() + ", ";
		    }

		    validTypes = validTypes.substring(0,
			    validTypes.length() - 2);
		}
    ...
    /**
     * Composite for editing an variable individual parameter. If the parameter
     * is contextable, the value of the context location where to find the
     * parameter may be specified. Also check if the parameter is a key-value
     * pair and shows the dialog regarding that
     * 
	...
	public VarParameterComposite(Composite parent, int style,
		jbt.tools.bteditor.model.ConceptualBTNode.Parameter parameter,
		String initialValue, boolean initialFromContext,
		boolean isConstant, String defaultVarText,
		BTNode.VarParameter varNodeParameter) {
	    ...
	    this.isConstant
		    .setToolTipText("If activated, the \"Constant\" field is enabled. That means that the parameter is a special key-value type");

	    this.variableNameText = new Text(this, SWT.BORDER);
	    this.variableNameText.setText(defaultVarText == null ? ""
		    : defaultVarText);
		...
		public void handleEvent(Event event) {
		    ...
		    variableNameText
			    .setEnabled(!VarParameterComposite.this.isConstant
				    .getSelection());
		    setIsConstant(VarParameterComposite.this.isConstant
			    .getSelection());
		}
	    };
	...
	public Parameter getParameter() throws RuntimeException {
		...
		if (!BTNode.checkVarParameter(parameter,
			this.valueText.getText(),
			this.variableNameText.getText(),
			this.isConstant.getSelection(), tree)) {
		    throw new RuntimeException("Invalid parameter value ("
			    + this.valueText.getText() + ") for parameter "
			    + this.parameter.getName());
		}
	...
	/*
	 * Helper function that sets the Field Value of a Variable Node
	 * Parameter This function is called from the checkbox event listener
	 */
	...
	public void dragStart(DragSourceEvent event) {
	    List<BTNode> selection = ((IStructuredSelection) viewer
		    .getSelection()).toList();
	...
	public void dragSetData(DragSourceEvent event) {
	    if (BTNodeIndentifierTransfer.getInstance().isSupportedType(
		    event.dataType)) {
	...
	public void dragOver(DropTargetEvent event) {
		...
		if (node.getConceptualNode().getType()
			...
			if (node.getConceptualNode().getNumChildren() != -1) {
			    if (node.getConceptualNode().getNumChildren() <= node
				    .getNumChildren()) {
				event.detail = DND.DROP_NONE;
	...
	public void drop(DropTargetEvent event) {
		...
		ConceptualBTNode newConceptualNode = NodesLoader.getNode(
		 ...
		 * canceled.
		 * 
		 * If the target node is not the root, then proceed as usual:
		 * the dropped node is inserted as a child or as a sibling of
		 * the target node.
		 */
		if (selectedNode.getConceptualNode().getType()
			.equals(NodeInternalType.ROOT.toString())) {
			...
			if (newConceptualNode.getNumChildren() == 0) {
			    ...
			    BTNode newNode = tree.createNode(NodesLoader
				    .getNode(nodeIdentifier.getFirst(),
	...
	private boolean closeToBottom(TreeItem treeItem) {
	    ...
	    Point mappedCoords = display.map(null, viewer.getTree(),
		    cursorLocation);
	...
	BTEditorInput editorInput = new BTEditorInput(
		((BTEditorInput) getEditorInput()).getEditorID()
			+ File.pathSeparator + node.getID().toString(), false,
		true);

	BTEditor openEditor = (BTEditor) activePage.openEditor(editorInput,
		BTEditor.ID);
	...
	public void partClosed(IWorkbenchPart part) {
		...
		Collection<BTEditor> guardEditors = editorToClose.openGuardEditors
			.values();
		...
		if (editorToClose.isFromGuard()) {
		    BTEditorInput editorInput = (BTEditorInput) editorToClose
			    .getEditorInput();
		    String[] pieces = editorInput.getTreeName().split(
			    File.pathSeparator);
		    long parentEditorID = Long.parseLong(pieces[0]);
		    BTEditor parentEditor = Utilities
			    .getBTEditor(parentEditorID);
		    if (parentEditor != null) {
			parentEditor.openGuardEditors
				.remove(editorToClose.guardNode);
		    }
		}
	    }
	}
     ...
     * edited. By doing so, this editor will be considered not to come from a
     * guard (<code>setIsFromGuard(false)</code>), so it will change its title
     * image to that of a normal BT. Also, {@link BTEditor#guardNode} and
